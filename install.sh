...
say ""
say "    https://${IP:-АДРЕС-СЕРВЕРА}:${WEB_PORT:-8443}/setup?token=$TOKEN"
say ""
say "Браузер предупредит о сертификате — это нормально, коробка выпускает"
say "его сама. Нажмите «Дополнительно» → «Всё равно перейти»."
say ""
if [ "$MODE_NOW" = "lan-gateway" ]; then
    ...
    say "доступна и через WireGuard: https://10.99.0.1:${WEB_PORT:-8443}"
fi
